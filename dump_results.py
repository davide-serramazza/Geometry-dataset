...
def dump_results(examples):
    ...
    with open("my_dataset_sentences.txt", "w+") as sen_file:
        for n_figs in examples.keys():
                ...
                dump_examples("test",(0,n_test),n_figs, bucket, sen_file,d)
                dump_examples("val",(n_test,2*n_test),n_figs, bucket, sen_file,d)
                dump_examples("train",(2*n_test,all_dataset_length  ),n_figs, bucket, sen_file,d)
    with open("all_captions.json","w+") as f:
        json.dump(d,f)
    sen_file.close()


def dump_examples(dataset_part,ranges,n_figs, examples, sen_file,d):
    for i in tqdm(range(ranges[0],ranges[1])):
        name = str(n_figs).zfill(2)+ "_" + str(i).zfill(5)
        sen = examples["sens"][i]
        img = examples["imgs"][i]
